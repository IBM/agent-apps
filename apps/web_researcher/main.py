...
Run:
    python main.py
    python main.py --port 18798
    python main.py --provider anthropic

Then open: http://127.0.0.1:18798
...
# ---------------------------------------------------------------------------
# Web search tool (Tavily — no cuga_channels dependency)
# ---------------------------------------------------------------------------
...
def _make_tools():
    ...
    def web_search(query: str, max_results: int = 6) -> str:
        ...
        if not api_key:
            return _json.dumps({"error": "TAVILY_API_KEY not set. Add it in the credentials panel."})
        try:
...
if __name__ == "__main__":
    ...
    if not os.getenv("TAVILY_API_KEY"):
        print("  ⚠️  TAVILY_API_KEY not set — add it in the Credentials panel in the UI.\n")
