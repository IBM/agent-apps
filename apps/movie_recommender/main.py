...
def _make_tools():
    ...
    async def lookup_movie(title: str) -> str:
        """
        Look up information about a movie using the Wikipedia REST API.
        Returns a summary including the plot, director, cast, release year, and genre.
        Use this to verify movie details before making recommendations or when the user
        asks about a specific film.

        Args:
            title: The movie title to look up (e.g. "Inception", "The Godfather")
        """
        # Wikipedia search first to find canonical page title
        ...
        try:
            ...
            return (
                f"Title: {data.get('title', page_title)}\n"
                f"Description: {data.get('description', 'N/A')}\n"
                f"Summary: {data.get('extract', 'No summary available.')[:1500]}"
            )
        except httpx.RequestError as e:
            return f"Network error looking up '{title}': {e}"
        except Exception as e:
            return f"Error looking up '{title}': {e}"
    ...
    return [lookup_movie, save_preference, get_preferences, save_recommendations]
...
2. Optionally call lookup_movie(title=...) to verify details about movies you plan to suggest.
