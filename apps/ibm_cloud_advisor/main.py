...
def _make_tools():
    ...
    def search_ibm_docs(query: str) -> str:
        """
        Search IBM Cloud documentation and architecture patterns.
        Use this to find pricing info, capability comparisons, architecture
        guides, and best practices for IBM services.

        Args:
            query: Search terms, e.g. "IBM Code Engine vs Kubernetes IKS comparison",
                   "IBM Event Streams Kafka pricing", "IBM Cloud Object Storage tiers".

        Returns:
            JSON with relevant excerpts and URLs from ibm.com documentation.
            Returns an error message if TAVILY_API_KEY is not set.
        """
        api_key = os.getenv("TAVILY_API_KEY")
        if not api_key:
            return json.dumps({
                "error": "TAVILY_API_KEY not set — doc search unavailable. "
                         "Rely on catalog results and training knowledge."
            })
        try:
            ...
            log.info("search_ibm_docs(%r): %d sources", query, len(sources))
            return json.dumps({"query": query, "sources": sources})
        except Exception as exc:
            return json.dumps({"error": str(exc)})

    return [search_ibm_catalog, search_ibm_docs]
...
2. Optionally call `search_ibm_docs` to find architecture patterns, pricing tiers,
