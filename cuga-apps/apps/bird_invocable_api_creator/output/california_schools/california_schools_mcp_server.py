...
@mcp.tool()
def count_non_charter_schools_by_county_with_free_meal_percent_below(county_name, max_percent) -> str:
    """Count non‑charter schools in a specified county where the K‑12 eligible free‑meal percent (Free Meal Count (K‑12) * 100 / Enrollment (K‑12)) is less than a given threshold."""
    try:
        _ns = {}
        exec(compile("def run(conn, county_name, max_percent):\n        # Parameterized query; case\u2011insensitive string match.\n        row = conn.execute(\n            \"\"\"\n            SELECT COUNT(T2.School)\n            FROM frpm AS T1\n            INNER JOIN schools AS T2 ON T1.CDSCode = T2.CDSCode\n            WHERE T2.County = ? COLLATE NOCASE\n              AND T2.Charter = 0\n              AND CAST(T1.`Free Meal Count (K-12)` AS REAL) * 100.0 / T1.`Enrollment (K-12)` < ?\n            \"\"\",\n            (county_name, max_percent)\n        ).fetchone()\n        return {\"school_count\": row[0] if row else None}\n    ", "<tool>", "exec"), _ns)
        with _conn() as _c:
            out = _ns["run"](_c, county_name=county_name, max_percent=max_percent)
        return _ok(out)
    except Exception as _e:
        return _err(_e)

@mcp.tool()
def count_schools_by_charter_number_and_mailing_city(charter_number, mail_city) -> str:
    """Count schools whose CharterNum and mailing city match the supplied values."""
    try:
        _ns = {}
        exec(compile("def run(conn, charter_number, mail_city):\n        row = conn.execute(\n            \"SELECT COUNT(*) FROM schools \"\n            \"WHERE CharterNum = ? COLLATE NOCASE AND MailCity = ? COLLATE NOCASE\",\n            (charter_number, mail_city)\n        ).fetchone()\n        return {\"school_count\": row[0] if row else None}\n    ", "<tool>", "exec"), _ns)
        with _conn() as _c:
            out = _ns["run"](_c, charter_number=charter_number, mail_city=mail_city)
        return _ok(out)
    except Exception as _e:
        return _err(_e)
...
def format_lowest_latitude_school(city, low_grade, school_name) -> str:
    ...
    try:
        _ns = {}
        exec(compile("def run(conn, city, low_grade, school_name):\n    # Return rows matching the gold\u2011SQL shape\n    return {\"rows\": [[city, low_grade, school_name]]}", "<tool>", "exec"), _ns)
        with _conn() as _c:
            out = _ns["run"](_c, city=city, low_grade=low_grade, school_name=school_name)
        return _ok(out)
    except Exception as _e:
        return _err(_e)
...
@mcp.tool()
def get_admin_last_name_district_county_school_by_charter_number(charter_number) -> str:
    """Returns a single row [admin_last_name, district, county, school] for the school identified by a Charter number."""
    try:
        _ns = {}
        exec(compile("def run(conn, charter_number):\n        row = conn.execute(\n            \"SELECT AdmLName1, District, County, School FROM schools WHERE CharterNum = ? COLLATE NOCASE\",\n            (charter_number,)\n        ).fetchone()\n        if not row:\n            return []  # no matching school\n        return [[row[0], row[1], row[2], row[3]]]", "<tool>", "exec"), _ns)
        with _conn() as _c:
...
@mcp.tool()
def get_phone_and_ext_by_writing_rank(rank) -> str:
    """Phone number and extension of the school that ranks at the given 1‑based position when ordered by average SAT writing score (highest first). Returns a list of rows [[phone, ext]] to match the gold SQL output."""
    try:
        _ns = {}
...
@mcp.tool()
def get_school_with_highest_k12_enrollment_by_doc(doc) -> str:
    """School name with the highest K‑12 enrollment among schools having the specified DOC code."""
    try:
        _ns = {}
        exec(compile("def run(conn, doc):\n    row = conn.execute(\n        \"SELECT T2.School \"\n        \"FROM frpm AS T1 \"\n        \"INNER JOIN schools AS T2 ON T1.CDSCode = T2.CDSCode \"\n        \"WHERE T2.DOC = ? \"\n        \"ORDER BY T1.`Enrollment (K-12)` DESC \"\n        \"LIMIT 1\",\n        (doc,)\n    ).fetchone()\n    return {\"school\": row[0] if row else None}", "<tool>", "exec"), _ns)
        with _conn() as _c:
            out = _ns["run"](_c, doc=doc)
        return _ok(out)
    except Exception as _e:
        return _err(_e)
...
@mcp.tool()
def get_schools_by_nslp_provision_and_county(provision_status, county_code) -> str:
    """Return educational level name (EILName) and school name for schools matching a given NSLP provision status and county code."""
    try:
        _ns = {}
        exec(compile("def run(conn, provision_status, county_code):\n        cur = conn.execute(\n            \"SELECT T2.EILName, T2.School \"\n            \"FROM frpm AS T1 \"\n            \"INNER JOIN schools AS T2 ON T1.CDSCode = T2.CDSCode \"\n            \"WHERE T1.`NSLP Provision Status` = ? COLLATE NOCASE \"\n            \"AND T1.`County Code` = ?\",\n            (provision_status, county_code)\n        )\n        rows = cur.fetchall()\n        # Return rows directly to match gold\u2011SQL shape\n        return {\"rows\": rows}\n    ", "<tool>", "exec"), _ns)
        with _conn() as _c:
            out = _ns["run"](_c, provision_status=provision_status, county_code=county_code)
        return _ok(out)
    except Exception as _e:
        return _err(_e)
...
@mcp.tool()
def get_top_sat_ge1500_admins() -> str:
    """Full first and last names of up to three administrators for the school with the highest NumGE1500 (students scoring 1500+ on the SAT). Returns null for missing administrators."""
    try:
        _ns = {}
        exec(compile("def run(conn):\n        row = conn.execute(\n            \"SELECT T2.AdmFName1, T2.AdmLName1, \"\n            \"T2.AdmFName2, T2.AdmLName2, \"\n            \"T2.AdmFName3, T2.AdmLName3 \"\n            \"FROM satscores AS T1 \"\n            \"INNER JOIN schools AS T2 ON T1.cds = T2.CDSCode \"\n            \"ORDER BY T1.NumGE1500 DESC LIMIT 1\"\n        ).fetchone()\n        if not row:\n            return {\"rows\": []}\n        return {\"rows\": [[row[0], row[1], row[2], row[3], row[4], row[5]]]}\n    ", "<tool>", "exec"), _ns)
        with _conn() as _c:
            out = _ns["run"](_c)
        return _ok(out)
    except Exception as _e:
...
@mcp.tool()
def get_websites_by_admins(admin1_first, admin1_last, admin2_first, admin2_last) -> str:
    """Return the website URLs of schools whose primary administrator matches either of two given first/last name pairs."""
    try:
        _ns = {}
        exec(compile("def run(conn, admin1_first, admin1_last, admin2_first, admin2_last):\n        cur = conn.execute(\n            \"SELECT Website FROM schools WHERE (AdmFName1 = ? COLLATE NOCASE AND AdmLName1 = ? COLLATE NOCASE) OR (AdmFName1 = ? COLLATE NOCASE AND AdmLName1 = ? COLLATE NOCASE)\",\n            (admin1_first, admin1_last, admin2_first, admin2_last)\n        )\n        rows = cur.fetchall()\n        websites = [r[0] for r in rows if r[0] is not None]\n        return {\"websites\": websites}\n    ", "<tool>", "exec"), _ns)
        with _conn() as _c:
            out = _ns["run"](_c, admin1_first=admin1_first, admin1_last=admin1_last, admin2_first=admin2_first, admin2_last=admin2_last)
        return _ok(out)
    except Exception as _e:
        return _err(_e)
...
@mcp.tool()
def top_charter_schools_by_enrollment(min_enrollment, limit) -> str:
    """Return the top charter schools (Charter = 1) whose K‑12 enrollment exceeds a given threshold, ordered by enrollment descending."""
    try:
        _ns = {}
        exec(compile("def run(conn, min_enrollment, limit):\n        cur = conn.execute(\n            \"\"\"SELECT `School Name` AS school_name,\n                      `County Name` AS county_name\n               FROM schools\n               WHERE `Charter` = 1\n                 AND `Enrollment (K-12)` > ?\n               ORDER BY `Enrollment (K-12)` DESC\n               LIMIT ?\"\"\",\n            (min_enrollment, limit)\n        )\n        rows = cur.fetchall()\n        return {\"schools\": [{\"school_name\": r[0], \"county_name\": r[1]} for r in rows]}\n    ", "<tool>", "exec"), _ns)
        with _conn() as _c:
            out = _ns["run"](_c, min_enrollment=min_enrollment, limit=limit)
        return _ok(out)
    except Exception as _e:
        return _err(_e)
