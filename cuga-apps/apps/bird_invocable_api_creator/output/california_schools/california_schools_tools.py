...
# count_non_charter_schools_by_county_with_free_meal_percent_below — Count non‑charter schools in a specified county where the K‑12 eligible free‑meal percent (Free Meal Count (K‑12) * 100 / Enrollment (K‑12)) is less than a given threshold.
def run(conn, county_name, max_percent):
        ...
        return {"school_count": row[0] if row else None}
count_non_charter_schools_by_county_with_free_meal_percent_below = run; del run

# count_schools_by_charter_number_and_mailing_city — Count schools whose CharterNum and mailing city match the supplied values.
def run(conn, charter_number, mail_city):
        row = conn.execute(
        ...
        return {"school_count": row[0] if row else None}
count_schools_by_charter_number_and_mailing_city = run; del run
...
def run(conn, city, low_grade, school_name):
    ...
    return {"rows": [[city, low_grade, school_name]]}
format_lowest_latitude_school = run; del run
...
get_admin_last_name_district_county_school_by_charter_number = run; del run
...
# get_phone_and_ext_by_writing_rank — Phone number and extension of the school that ranks at the given 1‑based position when ordered by average SAT writing score (highest first). Returns a list of rows [[phone, ext]] to match the gold SQL output.
def run(conn, rank):
        offset = rank - 1
        cur = conn.execute(
...
get_phone_and_ext_by_writing_rank = run; del run
...
# get_school_with_highest_k12_enrollment_by_doc — School name with the highest K‑12 enrollment among schools having the specified DOC code.
def run(conn, doc):
    row = conn.execute(
        "SELECT T2.School "
        "FROM frpm AS T1 "
    ...
    return {"school": row[0] if row else None}
get_school_with_highest_k12_enrollment_by_doc = run; del run
...
# get_schools_by_nslp_provision_and_county — Return educational level name (EILName) and school name for schools matching a given NSLP provision status and county code.
def run(conn, provision_status, county_code):
        cur = conn.execute(
        ...
        return {"rows": rows}
get_schools_by_nslp_provision_and_county = run; del run
...
# get_top_sat_ge1500_admins — Full first and last names of up to three administrators for the school with the highest NumGE1500 (students scoring 1500+ on the SAT). Returns null for missing administrators.
def run(conn):
        row = conn.execute(
...
get_top_sat_ge1500_admins = run; del run

# get_websites_by_admins — Return the website URLs of schools whose primary administrator matches either of two given first/last name pairs.
def run(conn, admin1_first, admin1_last, admin2_first, admin2_last):
        cur = conn.execute(
        ...
        return {"websites": websites}
get_websites_by_admins = run; del run
...
# top_charter_schools_by_enrollment — Return the top charter schools (Charter = 1) whose K‑12 enrollment exceeds a given threshold, ordered by enrollment descending.
def run(conn, min_enrollment, limit):
        cur = conn.execute(
...
top_charter_schools_by_enrollment = run; del run
